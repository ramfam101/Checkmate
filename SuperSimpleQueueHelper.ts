	...
	getEscalationCheckJob = () => {
		return async () => {
			try {
				...
				const activeIncidents = await this.incidentsRepository.findByStatus(true);

				for (const incident of activeIncidents) {
					try {
						const success = await this.notificationsService.checkAndHandleEscalation(incident);
						if (success) {
				...
				this.logger.debug({
					message: `Escalation check job completed. Processed ${activeIncidents.length} active incidents`,
					service: SERVICE_NAME,
					method: "getEscalationCheckJob",
				});
			} catch (error: unknown) {
