...
export interface INotificationsService {
	...
	checkAndHandleEscalation: (incident: any) => Promise<boolean>;
...
export class NotificationsService implements INotificationsService {
	...
	checkAndHandleEscalation = async (incident: any): Promise<boolean> => {
		// If no escalation is configured, skip
		if (!incident || !incident.monitorId || !incident.teamId) {
			return false;
		}

		// Get the monitor to check escalation time
		const monitor = await this.monitorsRepository.findById(incident.monitorId, incident.teamId);
		if (!monitor || !monitor.escalationTime) {
			return false;
		}
		...
		// Check if incident duration exceeds escalation time
		const incidentStartTime = new Date(incident.startTime).getTime();
		const currentTime = Date.now();
		const incidentDurationMs = currentTime - incidentStartTime;
		const escalationTimeMs = monitor.escalationTime * 60 * 1000; // Convert minutes to ms

		if (incidentDurationMs < escalationTimeMs) {
			return false;
		}

		// Escalation should be sent
		this.logger.info({
			message: `Triggering escalation for incident ${incident.id} on monitor ${monitor.name}`,
			service: SERVICE_NAME,
			method: "checkAndHandleEscalation",
		});

		// Get the escalation notification channels
		...
		const notifications = await this.notificationsRepository.findNotificationsByIds(monitor.escalationNotificationIds);

		if (notifications.length === 0) {
			this.logger.warn({
				message: `Escalation notification channels not found for monitor ${monitor.id}`,
				service: SERVICE_NAME,
				method: "checkAndHandleEscalation",
			});
			return false;
		...
		// Create a minimal MonitorStatusResponse for escalation
		const escalationStatus = {
			...
			message: `Monitor escalation: still down after ${monitor.escalationTime} minute(s)`,
		...
		// Mark as escalation
		escalationMessage.metadata.isEscalation = true;

		// Send to all escalation notification channels
		const tasks = notifications.map((notification) =>
			this.send(notification, monitor, escalationStatus, escalationDecision, escalationMessage)
		);

		const outcomes = await Promise.all(tasks);
		const succeeded = outcomes.filter(Boolean).length > 0;

		if (succeeded) {
			this.logger.info({
				message: `Escalation notification sent successfully for incident ${incident.id}`,
				service: SERVICE_NAME,
				method: "checkAndHandleEscalation",
			});
		}
