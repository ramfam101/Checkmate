...
const CreateMonitorPage = () => {
	...
	const onSubmit = async (data: any) => {
		const typedData = data as MonitorFormData;
		let result;
		if (isEditMode && monitorId) {
			result = await patch(`/monitors/${monitorId}`, typedData);
		} else {
			result = await post("/monitors", typedData);
		}
												...
												{selectedNotifications.map((notification, index) => (
													...
													>
														...
														<IconButton
															...
															aria-label="Remove escalation notification channel"
														>
															<Trash2 size={16} />
														</IconButton>
														{index < selectedNotifications.length - 1 && <Divider />}
