					...
					<Stack spacing={theme.spacing(LAYOUT.MD)}>
						{escalationFields.map((field, index) => (
							...
							>
								...
								>
									<Controller
										name={`escalationRules.${index}.escalateAfterMinutes`}
										control={control}
								...
								<Controller
									name={`escalationRules.${index}.notificationChannelIds`}
									control={control}
									render={({ field: f }) => {
										const notificationOptions = (notifications ?? []).map((n) => ({
											...n,
						...
						<Button
							variant="outlined"
							onClick={() =>
								appendEscalation({ escalateAfterMinutes: 30, notificationChannelIds: [] })
							}
						>
