									...
									{selectedNotifications.length > 0 && (
										<Stack flex={1} width="100%">
											{selectedNotifications.map((notification, index) => (
												<Stack key={notification.id} direction="row" alignItems="center" width="100%">
						...
						<Controller
							...
							render={({ field, fieldState }) => (
								<TextField
									{...field}
									type="number"
									fieldLabel={t("pages.createMonitor.form.escalation.optionMinutes.label")}
									placeholder={t("pages.createMonitor.form.escalation.optionMinutes.placeholder")}
									fullWidth
									inputProps={{ min: 0 }}
									error={!!fieldState.error}
									helperText={fieldState.error?.message ?? ""}
								...
								return (
									<Stack spacing={theme.spacing(LAYOUT.SM)}>
										<Autocomplete
											...
											isOptionEqualToValue={(option, value) => option.id === value.id}
											fieldLabel={t("pages.createMonitor.form.escalation.optionNotifications.label")}
										/>
