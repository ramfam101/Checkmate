...
const CreateMonitorPage = () => {
	...
	return (
		...
		>
			...
			<ConfigBox
				title={t("pages.createMonitor.form.escalation-rules.title")}
				subtitle={t("pages.createMonitor.form.escalation-rules.description")}
				rightContent={
					<Stack spacing={theme.spacing(LAYOUT.MD)}>
						<Controller
							name="escalationWaitTime"
							control={control}
							render={({ field, fieldState }) => (
								<TextField
									{...field}
									...
									type="number"
									fieldLabel={t("pages.createMonitor.form.escalation-rules.option.time.label")}
									placeholder={t("pages.createMonitor.form.escalation-rules.option.time.placeholder")}
									fullWidth
									error={!!fieldState.error}
									helperText={fieldState.error?.message ?? ""}
								/>
								...
								return (
									<Stack spacing={theme.spacing(LAYOUT.MD)}>
										<Autocomplete
											...
											isOptionEqualToValue={(option, value) => option.id === value.id}
											fieldLabel={t("pages.createMonitor.form.escalation-rules.option.channels.label")}
										/>
										{selectedNotifications.length > 0 && (
											<Stack
												flex={1}
