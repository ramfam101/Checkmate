...
const CreateMonitorPage = () => {
    ...
    subtitle="If monitor stays down for the specified time, notify additional channels"
    rightContent={
        <Stack spacing={theme.spacing(LAYOUT.MD)}>
            <Controller
                name="escalationTime"
            ...
            <Controller
                ...
                control={control}
                render={({ field }) => {
                    // Reuse the same notificationOptions logic from the notifications section
                    const notificationOptions = (notifications ?? []).map((n) => ({
                        ...n,
                        name: n.notificationName,
                    }));
