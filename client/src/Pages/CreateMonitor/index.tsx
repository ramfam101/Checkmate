...
const CreateMonitorPage = () => {
	...
	const onSubmit = async (data: MonitorFormData) => {
		const payload: MonitorFormData = {
			...data,
			escalationNotificationChannel: data.escalationNotificationChannel || undefined,
		};
						...
						rightContent={
							<Stack spacing={theme.spacing(LAYOUT.MD)}>
									...
									render={({ field, fieldState }) => (
										<TextField
											...
											onChange={(e) => {
												...
												field.onChange(Number.isNaN(value) ? 0 : Math.max(0, value));
											}}
											fieldLabel={t(
												"pages.createMonitor.form.notifications.option.escalationAfterMinutes.label"
											)}
											placeholder={t(
												"pages.createMonitor.form.notifications.option.escalationAfterMinutes.placeholder"
											)}
											helperText={
												fieldState.error?.message ||
												t(
													"pages.createMonitor.form.notifications.option.escalationAfterMinutes.helpText"
												)
											}
											error={!!fieldState.error}
											fullWidth
								...
								<Controller
									...
									control={control}
									render={({ field }) => {
										const emailChannels = (notifications ?? []).filter((n) => n.type === "email");
										return (
											<Select
												{...field}
												value={field.value ?? ""}
												...
												fieldLabel={t(
													"pages.createMonitor.form.notifications.option.escalationChannel.label"
												)}
											>
												<MenuItem value="">
													{t(
														"pages.createMonitor.form.notifications.option.escalationChannel.none"
													)}
												</MenuItem>
												{emailChannels.map((notification) => (
													<MenuItem
														key={notification.id}
