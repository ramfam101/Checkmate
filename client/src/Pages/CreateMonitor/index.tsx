...
const CreateMonitorPage = () => {
	...
	return (
		...
		>
			...
			{/* escalation */}
			<ConfigBox title={t("pages.createMonitor.form.escalation.title")} subtitle={t("pages.createMonitor.form.escalation.description")} rightContent={
				<Stack spacing={theme.spacing(LAYOUT.MD)}>
					<Controller name="escalateAfter" control={control} render={({field, fieldState}) => (
						<TextField
							{...field}
							value={field.value === 0 ? "" : field.value}
							onChange={(e) => {
					...
					<Controller name="escalationChannels" control={control} render={({field}) => {
						const notificationOptions = (notifications ?? []).map((n) => ({
							...n,
							name: n.notificationName,
						}));
