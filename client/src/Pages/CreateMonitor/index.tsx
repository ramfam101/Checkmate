...
const CreateMonitorPage = () => {
	...
	useEffect(() => {
		console.log("Defaults being set:", defaults);
		form.reset(defaults);
	}, [defaults, form]);
					...
					<Controller
							...
							render={({ field, fieldState }) => (
								<TextField
									...
									type="number"
									fieldLabel={t("pages.createMonitor.form.escalation.options.time.label")}
									placeholder={"0"}
									fullWidth
									error={!!fieldState.error}
									helperText={fieldState.error?.message ?? ""}
						...
						render={({ field }) => {
							...
							return (
								<Stack spacing={theme.spacing(SPACING.MD)}>
									<FieldLabel>
                                        {t("pages.createMonitor.form.escalation.options.channels.label")}
                                    </FieldLabel>
									<Autocomplete
										multiple
										options={notificationOptions}
