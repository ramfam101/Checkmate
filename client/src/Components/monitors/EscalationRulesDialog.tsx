...
export const EscalationRulesDialog: React.FC<EscalationRulesDialogProps> = ({
	...
	const resetNewRule = () => {
		setNewRule({ delayMinutes: 15, notificationId: "", escalationChannelId: "" });
		setErrors({});
	};
...
{availableNotifications.length === 0 ? (
						...
						<>
							<Select
								...
								size="small"
								displayEmpty
								value={newRule.notificationId || ""}
								onChange={(e) => setNewRule({ ...newRule, notificationId: e.target.value })}
								error={!!errors.notificationId}
								sx={{ mb: 2 }}
							>
								<MenuItem value="">
									{t("pages.monitor.escalations.columns.notification", "Select Source Notification")}
								</MenuItem>
								{availableNotifications.map((notification) => (
									<MenuItem key={notification.id} value={notification.id}>
										{notification.notificationName}
