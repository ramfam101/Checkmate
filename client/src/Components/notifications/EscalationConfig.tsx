...
export const EscalationConfig: React.FC<EscalationConfigProps> = ({
	...
	const [enabled, setEnabled] = useState(notification?.escalationConfig?.enabled || false);
	const [delayMinutes, setDelayMinutes] = useState(notification?.escalationConfig?.delayMinutes || 15);
	...
	const otherNotifications = availableNotifications.filter((n) => n.id !== notification?.id);
		...
		<Card variant="outlined" sx={{ mt: 3, bgcolor: "background.paper" }}>
			<CardContent>
				<Box sx={{ mb: 2 }}>
					...
					/>
					<FormHelperText>
						Automatically escalate to a secondary notification channel if an incident remains unacknowledged
					</FormHelperText>
				</Box>
				...
				{enabled && (
					<>
						...
						<Grid container spacing={2}>
							...
							<Grid item xs={12} sm={6}>
								<FormControl fullWidth error={enabled && !escalationChannelId}>
									<Select
							...
							<Grid item xs={12}>
								<Box sx={{ p: 2, bgcolor: "background.default", borderRadius: 1 }}>
									<Typography variant="caption" color="textSecondary">
										Example: If no one acknowledges this incident within {delayMinutes} minutes, an
										escalation notification will be sent to{" "}
										{escalationChannelId
											? otherNotifications.find((n) => n.id === escalationChannelId)?.notificationName
