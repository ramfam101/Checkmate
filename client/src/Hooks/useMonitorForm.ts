...
const getBaseDefaults = (data?: Monitor | null) => ({
	...
	escalatedNotification: {
		enabled: data?.escalatedNotification?.enabled ?? false,
		escalateAfterMinutes: data?.escalatedNotification?.escalateAfterMinutes ?? 30,
		notificationIds: data?.escalatedNotification?.notificationIds || [],
	},
