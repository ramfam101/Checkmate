...
import type { Incident, Monitor } from "../src/types/index.ts";
...
const createService = ({
	...
	const notificationsRepository = {
		findNotificationsByIds: jest.fn().mockResolvedValue([{ id: "n2", type: "email", notificationName: "Escalation Email" }]),
		create: jest.fn(),
		findById: jest.fn(),
...
describe("NotificationsService", () => {
	...
	it("does not resend escalation notifications when the incident was already escalated", async () => {
		...
		const result = await service.handleEscalationNotifications(monitor, {
			status: false,
			code: 500,
			message: "Down",
		} as any);
