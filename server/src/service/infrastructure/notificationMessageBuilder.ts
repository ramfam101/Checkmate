...
export class NotificationMessageBuilder implements INotificationMessageBuilder {
	...
	private determineNotificationType(decision: MonitorActionDecision, monitor: Monitor): NotificationType {
		// Escalation notification (has highest priority over standard down)
	...
	private buildEscalationContent(monitor: Monitor, monitorStatusResponse: MonitorStatusResponse): NotificationContent {
		...
		const summary = `Monitor "${monitor.name}" has been down for longer than the configured escalation threshold and requires immediate attention.`;
		const details = [`URL: ${monitor.url}`, `Status: Down (Escalated)`, `Type: ${monitor.type}`];
