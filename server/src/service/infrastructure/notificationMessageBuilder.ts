...
export class NotificationMessageBuilder implements INotificationMessageBuilder {
	...
	buildMessage(
		...
		return {
			...
			metadata: {
				...
				escalationStep: decision.escalationStep ?? null,
			},
		};
	}
	...
	private buildMonitorDownContent(
		...
		const title = isEscalation ? `Still Down: ${monitor.name}` : `Monitor Down: ${monitor.name}`;
		const summary = isEscalation
			? `Monitor "${monitor.name}" is still down and unreachable.`
