...
export class NotificationMessageBuilder implements INotificationMessageBuilder {
	...
	private buildContent(
		...
		switch (type) {
			case "monitor_down":
				return this.buildMonitorDownContent(monitor, monitorStatusResponse, decision.isEscalation ?? false);
			case "monitor_up":
	...
	private buildMonitorDownContent(
		...
		const title = isEscalation ? `Escalation: Monitor ${monitor.name} is still down` : `Monitor Down: ${monitor.name}`;
		const summary = isEscalation
			? `Monitor "${monitor.name}" is still down and unreachable after escalation delay.`
