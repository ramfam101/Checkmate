...
export class NotificationMessageBuilder implements INotificationMessageBuilder {
	...
	buildEscalationMessage(
		...
		const severity = this.determineSeverity(type);
		const content = this.buildEscalationContent(monitor, monitorStatusResponse, delayMinutes);
	...
	private buildEscalationContent(monitor: Monitor, monitorStatusResponse: MonitorStatusResponse, delayMinutes: number): NotificationContent {
		const title = `ESCALATION: ${monitor.name} still down after ${delayMinutes} minutes`;
		const summary = `Monitor "${monitor.name}" is still down and unreachable after ${delayMinutes} minutes.`;
		const details = [
