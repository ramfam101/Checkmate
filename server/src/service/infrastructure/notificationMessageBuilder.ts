...
export class NotificationMessageBuilder implements INotificationMessageBuilder {
    ...
    buildMessage(
        ...
        const type = this.determineNotificationType(decision, monitor, isEscalation);
    ...
    private determineNotificationType(
        ...
        if (monitor.status === "down" && isEscalation) {
            return "monitor_down_escalated";
        }
    ...
    private buildContent(type: NotificationType, monitor: Monitor, monitorStatusResponse: MonitorStatusResponse): NotificationContent {
        switch (type) {
            ...
            case "monitor_down_escalated":
                return this.buildMonitorDownEscalatedContent(monitor, monitorStatusResponse);
            case "monitor_up":
    ...
    private buildMonitorDownEscalatedContent(monitor: Monitor, monitorStatusResponse: MonitorStatusResponse): NotificationContent {
        ...
        const details = [
            ...
            `Status: Down`,
            `Type: ${monitor.type}`,
            `Escalation Delay: ${monitor.escalationAfterMinutes} minute(s)`,
        ];
