...
export class NotificationMessageBuilder implements INotificationMessageBuilder {
	...
	private buildMonitorDownContent(monitor: Monitor, monitorStatusResponse: MonitorStatusResponse, decision?: MonitorActionDecision): NotificationContent {
		const isEscalation = decision?.notificationReason === "escalation";
		const title = isEscalation ? `ESCALATION: ${monitor.name} is still down` : `Monitor Down: ${monitor.name}`;
		const summary = isEscalation ? `Monitor "${monitor.name}" is still down and unreachable.` : `Monitor "${monitor.name}" is currently down and unreachable.`;
