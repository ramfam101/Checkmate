...
export class NotificationMessageBuilder implements INotificationMessageBuilder {
	...
	private buildMonitorDownContent(monitor: Monitor, monitorStatusResponse: MonitorStatusResponse, isEscalation = false): NotificationContent {
		// my new code, depending on whether the email is escalation or not, the email contains different wording
		const title = isEscalation ? `Escalation: Monitor ${monitor.name} still down` : `Monitor Down: ${monitor.name}`;

		const summary = isEscalation
			? `Escalation alert: Monitor "${monitor.name}" is still down after the configured delay.`
