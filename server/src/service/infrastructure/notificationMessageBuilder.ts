...
export class NotificationMessageBuilder implements INotificationMessageBuilder {
	...
	buildMessage(
		...
		return {
			...
			metadata: {
				...
				downDurationMinutes: decision.escalationDurationMinutes,
			},
		};
	}
	...
	private buildContent(
		...
		switch (type) {
			...
			case "monitor_down_escalated":
				return this.buildEscalatedMonitorDownContent(monitor, monitorStatusResponse, decision);
			case "monitor_up":
	...
	private buildEscalatedMonitorDownContent(
		monitor: Monitor,
		monitorStatusResponse: MonitorStatusResponse,
		decision: MonitorActionDecision
	): NotificationContent {
		const downDurationMinutes = decision.escalationDurationMinutes ?? 0;
		const title = `Escalation: Monitor ${monitor.name} still down`;
		const summary = `Monitor "${monitor.name}" is still down.${downDurationMinutes > 0 ? ` Down for ${downDurationMinutes} minutes.` : ""}`;
		const details = [`URL: ${monitor.url}`, `Status: Down`, `Type: ${monitor.type}`];
