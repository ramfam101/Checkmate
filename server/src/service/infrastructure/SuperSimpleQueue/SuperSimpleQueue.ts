...
export class SuperSimpleQueue implements ISuperSimpleQueue {
	...
	init = async () => {
		try {
			...
			this.scheduler.addTemplate("escalation-cleanup", this.helper.getEscalationCleanupJob());
			...
			this.scheduler.addJob({ id: "escalation-check", template: "escalation-cleanup", active: true, repeat: 60 * 1000 });
