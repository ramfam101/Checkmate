...
export class SuperSimpleQueue implements ISuperSimpleQueue {
	...
	init = async () => {
		try {
			...
			this.scheduler.addTemplate("escalation-notifications", this.helper.getEscalationNotificationsJob());
			...
			this.scheduler.addJob({ id: "escalation-notifications", template: "escalation-notifications", active: true, repeat: 5 * 60 * 1000 }); // Run every 5 minutes
