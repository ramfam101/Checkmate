...
export class SuperSimpleQueue implements ISuperSimpleQueue {
	...
	init = async () => {
		try {
			...
			this.scheduler.addTemplate("escalation-sweep", this.helper.getEscalationSweepJob());
			...
			this.scheduler.addJob({ id: "escalation-sweep", template: "escalation-sweep", active: true, repeat: 60 * 1000 });
