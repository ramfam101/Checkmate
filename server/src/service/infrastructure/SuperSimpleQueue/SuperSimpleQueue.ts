...
export class SuperSimpleQueue implements ISuperSimpleQueue {
	...
	init = async () => {
		try {
			...
			this.scheduler.addTemplate("incident-reminder", this.helper.getIncidentReminderJob());
			...
			this.scheduler.addJob({ id: "incident-reminder", template: "incident-reminder", active: true, repeat: 60 * 1000 }); // Run every 1 minute to support monitor escalation intervals
