    ...
    getHeartbeatJob = () => {
        return async (monitor: Monitor) => {
            try {
                ...
                // Load any currently active incident first
                let activeIncident = await this.incidentsRepository.findActiveByMonitorId(monitorId, teamId);
                let incident = activeIncident;
                try {
                    const handledIncident = await this.incidentService.handleIncident(
                    ...
                    if (handledIncident) {
                        activeIncident = handledIncident;
                        incident = handledIncident;
                    }
                } catch (error: unknown) {
                    this.logger.warn({
                ...
                if (statusChangeResult.monitor.status === "up" && decision.shouldSendNotification) {
                    this.notificationsService.handleNotifications(statusChangeResult.monitor, status, decision).catch((error: unknown) => {
                        this.logger.error({
                            message: `Error sending recovery notifications for job ${statusChangeResult.monitor.id}: ${error instanceof Error ? error.message : "Unknown error"}`,
                            service: SERVICE_NAME,
                            method: "getMonitorJob",
                            stack: error instanceof Error ? error.stack : undefined,
                        });
                    ...
                    try {
                        this.logger.debug({
                            message: `Checking escalation for monitor ${statusChangeResult.monitor.id}, incident ${activeIncident.id}`,
                            service: SERVICE_NAME,
                            method: "getMonitorJob",
                        });

                        const escalationNotificationIds = await this.notificationsService.getEscalationNotificationIds(
                            statusChangeResult.monitor,
                            activeIncident
                        );

                        this.logger.debug({
                            message: `Found ${escalationNotificationIds.length} escalation notifications`,
                            service: SERVICE_NAME,
                            method: "getMonitorJob",
                        });
                        ...
                        if (escalationNotificationIds.length > 0) {
                            const escalationDecision: MonitorActionDecision = {
                                ...
                                incidentReason:
                                    statusChangeResult.monitor.status === "breached" ? "threshold_breach" : "status_down",
                                notificationReason:
                                    statusChangeResult.monitor.status === "breached" ? "threshold_breach" : "status_change",
                                thresholdBreaches: statusChangeResult.thresholdBreaches,
                            };

                            const escalationSent = await this.notificationsService.handleNotifications(
                                statusChangeResult.monitor,
                                status,
                                escalationDecision,
                            ...
                            this.logger.debug({
                                message: `Escalation notifications sent: ${escalationSent}`,
                                service: SERVICE_NAME,
                                method: "getMonitorJob",
                            });

                            if (escalationSent) {
                                const updatedSentNotificationIds = [
                                    ...(activeIncident.sentNotificationIds ?? []),
                                    ...escalationNotificationIds,
                                ].filter((value, index, self) => self.indexOf(value) === index);

                                await this.incidentsRepository.updateById(activeIncident.id, teamId, {
                                    sentNotificationIds: updatedSentNotificationIds,
                                });

                                this.logger.debug({
                                    message: `Updated incident ${activeIncident.id} with sent notification IDs`,
                                    service: SERVICE_NAME,
                                    method: "getMonitorJob",
                                });
                            }
                        ...
                        this.logger.error({
                            message: `Error in escalation logic: ${escalationError instanceof Error ? escalationError.message : "Unknown error"}`,
                            service: SERVICE_NAME,
                            method: "getMonitorJob",
