...
export class SuperSimpleQueueHelper implements ISuperSimpleQueueHelper {
	...
	getHeartbeatJob = () => {
		return async (monitor: Monitor) => {
			try {
				...
				// Step 7. Handle escalation notifications (best effort, don't wait)
				this.handleEscalationNotifications(statusChangeResult, status).catch((error: unknown) => {
					this.logger.error({
	...
	private async handleEscalationNotifications(statusChangeResult: StatusChangeResult, monitorStatusResponse: MonitorStatusResponse) {
		...
		const sentEscalations = activeIncident.escalationsSent ?? [];

		const dueRules = escalationRules
			.map((rule, index) => ({ rule, index }))
			.filter(({ rule, index }) => elapsedMinutes >= rule.afterMinutes && !sentEscalations.includes(index));

		if (dueRules.length === 0) {
			return;
		}

		const notificationIds = Array.from(new Set(dueRules.flatMap(({ rule }) => rule.notificationIds ?? [])));
		if (notificationIds.length === 0) {
			return;
		}

		const success = await this.notificationsService.sendEscalationNotifications(monitor, monitorStatusResponse, notificationIds);
		if (!success) {
			return;
		}

		await this.incidentsRepository.updateById(activeIncident.id, activeIncident.teamId, {
			escalationsSent: [...sentEscalations, ...dueRules.map(({ index }) => index)],
		});
	}
