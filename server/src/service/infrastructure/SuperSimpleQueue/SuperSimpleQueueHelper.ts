...
export interface ISuperSimpleQueueHelper {
	...
	getEscalationNotificationsJob(): () => Promise<void>;
...
export class SuperSimpleQueueHelper implements ISuperSimpleQueueHelper {
	...
	getEscalationNotificationsJob = () => {
		return async () => {
			try {
				this.logger.info({
					message: "Starting escalation notifications job",
					service: SERVICE_NAME,
					method: "getEscalationNotificationsJob",
				});

				await this.escalationService.processEscalationNotifications();

				this.logger.info({
					message: "Completed escalation notifications job",
					service: SERVICE_NAME,
					method: "getEscalationNotificationsJob",
				});
			} catch (error: unknown) {
				this.logger.error({
					message: `Error in escalation notifications job: ${error instanceof Error ? error.message : "Unknown error"}`,
					service: SERVICE_NAME,
					method: "getEscalationNotificationsJob",
					stack: error instanceof Error ? error.stack : undefined,
				});
