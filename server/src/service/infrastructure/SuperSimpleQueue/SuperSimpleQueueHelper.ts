	...
	getHeartbeatJob = () => {
		return async (monitor: Monitor) => {
			try {
				...
				// Step 8. Handle escalated notifications (best effort, don't wait)
				this.notificationsService.handleEscalatedNotifications(statusChangeResult.monitor, status).catch((error: unknown) => {
					this.logger.warn({
