...
export class EmailProvider implements INotificationProvider {
    ...
    async sendMessage(notification: Notification, message: NotificationMessage): Promise<boolean> {
        ...
        try {
            ...
            this.logger.info({
                ...
                monitorName: message.monitor.name,
                isEscalation: message.isEscalation,
                escalationDelayMinutes: message.escalationDelayMinutes,
            });
    ...
    private buildSubject(message: NotificationMessage): string {
        if (message.isEscalation) {
            const delayText = message.escalationDelayMinutes
                ? ` - Escalation after ${message.escalationDelayMinutes} minutes`
                : " - Escalation";
            return `Escalation: Monitor ${message.monitor.name} is down${delayText}`;
        }
    ...
    private async buildEmailFromMessage(message: NotificationMessage): Promise<string | undefined> {
        const escalationNote = message.isEscalation
            ? `This is an escalation alert. The monitor has been down for ${message.escalationDelayMinutes} minutes.`
            : "";
        ...
        this.logger.debug({
            message: "Building email from message",
            service: SERVICE_NAME,
