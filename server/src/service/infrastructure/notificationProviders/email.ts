...
export class EmailProvider implements INotificationProvider {
	...
	private buildSubject(message: NotificationMessage): string {
		const subjectPrefix = message.metadata.isEscalation ? "Escalated: " : "";
		switch (message.type) {
			case "monitor_down":
				return `${subjectPrefix}Monitor ${message.monitor.name} is down`;
			case "monitor_up":
			...
			default:
				return `${subjectPrefix}Alert: ${message.monitor.name}`;
		}
	}
