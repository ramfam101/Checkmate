...
export class EmailProvider implements INotificationProvider {
	...
	private buildSubject(message: NotificationMessage): string {
		const isEscalation = message.metadata.isEscalation === true;
		...
		if (isEscalation) {
			switch (message.type) {
				...
				case "threshold_breach":
					return `Escalation: Monitor ${message.monitor.name} threshold still exceeded`;
				default:
					return `Escalation: ${message.monitor.name}`;
			}
		}
