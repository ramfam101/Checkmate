...
export interface INotificationsService {
	...
	// Send specific notification IDs for a given monitor (used for escalation/testing)
	sendEscalationNotifications: (notificationIds: string[], monitor: Monitor) => Promise<boolean>;
}
...
export class NotificationsService implements INotificationsService {
	...
	private send = async (
		...
		this.logger.info({
			message: "Preparing to send notification",
			service: SERVICE_NAME,
			method: "send",
			details: { notificationId: notification.id, type: notification.type, monitorId: monitor.id },
		});
	...
	private sendNotifications = async (monitor: Monitor, monitorStatusResponse: MonitorStatusResponse, decision: MonitorActionDecision) => {
		...
		this.logger.info({
			...
			details: {
				...
				messageTitle: notificationMessage.content?.title,
			},
		});
	...
	// Send specific notifications (by id) using the notification message builder for the provided monitor.
	sendEscalationNotifications = async (notificationIds: string[], monitor: Monitor) => {
		if (!notificationIds || notificationIds.length === 0) return false;

		// Build a simple MonitorStatusResponse and decision to generate a notification message
		...
		// Mark this message as an escalation so templates/subjects can include ESCALATION prefix
		try {
			notificationMessage = {
				...notificationMessage,
				content: {
					...
					summary: `ESCALATION: ${notificationMessage.content.summary}`,
				},
			};
		} catch (e) {
			this.logger.warn({ message: "Failed to mark escalation message", service: SERVICE_NAME, method: "sendEscalationNotifications" });
		}

		const tasks: Promise<boolean>[] = [];
		for (const id of notificationIds) {
			try {
				const notification = await this.notificationsRepository.findById(id, monitor.teamId);
				if (notification) {
					tasks.push(this.send(notification, monitor, monitorStatusResponse, decision, notificationMessage));
				}
			} catch (err) {
				this.logger.warn({
					message: `Failed to load notification ${id} for escalation`,
					service: SERVICE_NAME,
		...
		const outcomes = await Promise.all(tasks);
		const succeeded = outcomes.filter(Boolean).length;
		return succeeded === outcomes.length && outcomes.length > 0;
	};
