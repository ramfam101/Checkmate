...
export interface INotificationsService {
	...
	sendEscalationNotifications: (monitor: Monitor, notificationIds: string[]) => Promise<boolean>;
...
export class NotificationsService implements INotificationsService {
	...
	sendEscalationNotifications = async (monitor: Monitor, notificationIds: string[]) => {
		const notifications = await this.notificationsRepository.findNotificationsByIds(notificationIds);
		...
		const tasks = notifications.map((notification) => this.send(notification, monitor, {} as any, { shouldSendNotification: true } as any, escalationMessage));
