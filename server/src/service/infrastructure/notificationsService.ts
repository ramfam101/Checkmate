...
export class NotificationsService implements INotificationsService {
	...
	handleEscalatedNotifications = async (monitor: Monitor, monitorStatusResponse: MonitorStatusResponse, decision: MonitorActionDecision) => {
		...
		const clientHost = settings.clientHost || "Host not defined";
		const notificationMessage = this.notificationMessageBuilder.buildMessage(monitor, monitorStatusResponse, decision, clientHost);
		// Override the title to indicate escalation
		notificationMessage.content.title = `Escalation: ${monitor.name} is still down`;
		notificationMessage.content.summary = `Monitor "${monitor.name}" has been down and has passed its escalation threshold.`;
		const tasks = notifications.map((notification) => this.send(notification, monitor, monitorStatusResponse, decision, notificationMessage));
		const outcomes = await Promise.all(tasks);
		const succeeded = outcomes.filter(Boolean).length;
		return succeeded > 0;
