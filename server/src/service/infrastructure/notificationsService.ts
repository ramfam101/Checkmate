...
export class NotificationsService implements INotificationsService {
	...
	handleEscalationNotifications = async (monitor: Monitor, monitorStatusResponse: MonitorStatusResponse) => {
		...
		if (escalationMinutes == null || escalationNotifications.length === 0) {
			return false;
		}
		...
		const incidentAgeMs = Date.now() - new Date(activeIncident.startTime).getTime();
		if (incidentAgeMs < escalationMinutes * 60 * 1000) {
			return false;
		}
		...
		const sent = await this.sendNotifications(escalationNotifications, monitor, monitorStatusResponse, decision);
		if (sent) {
