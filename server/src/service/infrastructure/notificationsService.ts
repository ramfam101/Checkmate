...
export interface INotificationsService {
	...
	handleEscalationNotification: (monitor: Monitor, incident: Incident) => Promise<boolean>;
...
export class NotificationsService implements INotificationsService {
	...
	handleEscalationNotification = async (monitor: Monitor, incident: Incident) => {
		if (!monitor.escalation || !monitor.escalation.notificationIds || monitor.escalation.notificationIds.length === 0) {
			return false;
		}

		if (incident.escalationSent) {
			return false; // Already sent escalation notification for this incident
		}

		const notifications = await this.notificationsRepository.findNotificationsByIds(monitor.escalation.notificationIds);
