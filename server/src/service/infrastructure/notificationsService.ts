...
export interface INotificationsService {
	...
	sendEscalationNotification: (monitor: Monitor, incident: Incident) => Promise<boolean>;
...
export class NotificationsService implements INotificationsService {
	...
	sendEscalationNotification = async (monitor: Monitor, incident: Incident) => {
		if (!monitor.escalationNotificationId || !monitor.escalationDelayMinutes) {
			return false;
		}

		const notification = await this.notificationsRepository.findById(monitor.escalationNotificationId, monitor.teamId);
		const settings = this.settingsService.getSettings();
		const clientHost = settings.clientHost || "Host not defined";
		const notificationMessage = this.notificationMessageBuilder.buildEscalationMessage(monitor, incident, clientHost, monitor.escalationDelayMinutes);
		return await this.send(notification, notificationMessage);
	};
