...
class MongoMonitorsRepository implements IMonitorsRepository {
	...
	private toEntity = (doc: MonitorDocument): Monitor => {
		...
		return {
			...
			escalatedNotificationDelay: doc.escalatedNotificationDelay ?? 0,
			escalationNotificationChannels: (doc.escalationNotificationChannels ?? []).map((ch) => toStringId(ch)),
	...
	private toEntityWithChecks = (doc: MonitorDocument): Monitor => {
		...
		return {
			...
			notifications: notificationIds,
			escalatedNotificationDelay: doc.escalatedNotificationDelay ?? 0,
			escalationNotificationChannels: (doc.escalationNotificationChannels ?? []).map((ch: unknown) => toStringId(ch)),
			secret: doc.secret ?? undefined,
			cpuAlertThreshold: doc.cpuAlertThreshold,
			cpuAlertCounter: doc.cpuAlertCounter,
			memoryAlertThreshold: doc.memoryAlertThreshold,
