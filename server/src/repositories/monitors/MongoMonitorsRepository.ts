...
class MongoMonitorsRepository implements IMonitorsRepository {
	...
	private toEntity = (doc: MonitorDocument): Monitor => {
		...
		const escalationNotificationIds = (doc.notificationEscalationNotifications ?? []).map((notification) => toStringId(notification));
		...
		return {
			...
			notificationEscalationEnabled: doc.notificationEscalationEnabled ?? false,
			notificationEscalationDelayMinutes: doc.notificationEscalationDelayMinutes ?? 0,
			notificationEscalationNotifications: escalationNotificationIds,
	...
	private toEntityWithChecks = (doc: MonitorDocument): Monitor => {
		...
		const escalationNotificationIds = (doc.notificationEscalationNotifications ?? []).map((notification: unknown) => toStringId(notification));
