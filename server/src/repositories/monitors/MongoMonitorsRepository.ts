...
class MongoMonitorsRepository implements IMonitorsRepository {
	...
	private toEntity = (doc: MonitorDocument): Monitor => {
		...
		const escalatedNotifs = (doc.escalatedNotifications ?? []).map((escNotif) => ({
			notificationId: toStringId(escNotif.notificationId),
		...
		return {
			...
			escalatedNotifications: escalatedNotifs.length > 0 ? escalatedNotifs : undefined,
