...
class MongoMonitorsRepository implements IMonitorsRepository {
	...
	private toEntity = (doc: MonitorDocument): Monitor => {
		...
		const escalationPolicy =
			doc.escalationPolicy?.map((step) => ({
				stepName: step.stepName ?? undefined,
				delayMinutes: step.delayMinutes,
				notificationIds: (step.notificationIds ?? []).map((notification) => toStringId(notification)),
			})) ?? [];
	...
	private toEntityWithChecks = (doc: MonitorDocument): Monitor => {
		...
		const escalationPolicy =
			doc.escalationPolicy?.map((step) => ({
				stepName: step.stepName ?? undefined,
				delayMinutes: step.delayMinutes,
				notificationIds: (step.notificationIds ?? []).map((notification: unknown) => toStringId(notification)),
			})) ?? [];
