...
class MongoMonitorsRepository implements IMonitorsRepository {
	...
	private toEntity = (doc: MonitorDocument): Monitor => {
		...
		const escalationNotifications = (doc.escalationNotifications ?? []).map((notification) => toStringId(notification));
		...
		return {
			...
			escalationAfter: doc.escalationAfter ?? 0,
			escalationNotifications: escalationNotifications,
			downtimeStartAt: doc.downtimeStartAt ? toDateString(doc.downtimeStartAt) : undefined,
	...
	private toEntityWithChecks = (doc: MonitorDocument): Monitor => {
		...
		const escalationNotifications = (doc.escalationNotifications ?? []).map((notification: unknown) => toStringId(notification));
