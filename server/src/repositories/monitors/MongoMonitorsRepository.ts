...
class MongoMonitorsRepository implements IMonitorsRepository {
	...
	private toEntity = (doc: MonitorDocument): Monitor => {
		...
		return {
			...
			escalateAfterMinutes: doc.escalateAfterMinutes ?? 30,
			escalationNotifications: escalationNotificationIds,
			currentIncidentStartedAt: doc.currentIncidentStartedAt ?? null,
			lastEscalatedIncidentStartedAt: doc.lastEscalatedIncidentStartedAt ?? null,
