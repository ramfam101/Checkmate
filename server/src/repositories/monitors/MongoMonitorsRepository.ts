...
class MongoMonitorsRepository implements IMonitorsRepository {
	...
	private toEntity = (doc: MonitorDocument): Monitor => {
		...
		const escalatedNotificationIds = (doc.escalatedNotifications ?? []).map((n) => toStringId(n));
	...
	private toEntityWithChecks = (doc: MonitorDocument): Monitor => {
		...
		const escalatedNotificationIds = (doc.escalatedNotifications ?? []).map((n: unknown) => toStringId(n));
