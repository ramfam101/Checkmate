...
class MongoMonitorsRepository implements IMonitorsRepository {
	...
	private toEntity = (doc: MonitorDocument): Monitor => {
		...
		const escalations = (doc as unknown as { escalations?: Array<{ delayMinutes: number; notifications: unknown[] }> }).escalations;
		...
		return {
			...
			escalations: (escalations ?? []).map((step) => ({
				delayMinutes: step.delayMinutes,
				notifications: (step.notifications ?? []).map((n) => toStringId(n)),
			})),
