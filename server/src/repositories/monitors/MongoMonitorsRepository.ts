...
class MongoMonitorsRepository implements IMonitorsRepository {
	...
	private toEntity = (doc: MonitorDocument): Monitor => {
		...
		return {
			...
			enableEscalation: doc.enableEscalation ?? false,
			escalationDelayMinutes: doc.escalationDelayMinutes ?? 10,
			escalationChannelId: doc.escalationChannelId ? toStringId(doc.escalationChannelId) : "",
