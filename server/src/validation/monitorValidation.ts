...
export const createMonitorBodyValidation = z.object({
	...
	escalationMinutes: z.array(z.number().int().min(1, "Escalation delay must be at least 1 minute")).max(10, "You can configure up to 10 escalation stages").optional(),
...
const importedMonitorSchema = z.object({
	...
	escalationMinutes: z.array(z.number().int().min(1, "Escalation delay must be at least 1 minute")).max(10, "You can configure up to 10 escalation stages").default([]),
	escalationNotificationIds: z.array(z.string()).default([]),
	secret: z.string().optional(),
