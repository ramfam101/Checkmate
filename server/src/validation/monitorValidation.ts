...
// --- Escalation Schemas (Unified to Zod) ---

const escalationSchema = z.object({
    delayMinutes: z.number().int().min(1).max(1440),
    escalationChannelId: z.string().regex(/^[0-9a-fA-F]{24}$/, "Invalid Channel ID"),
});
...
export const updateMonitorNotificationsSchema = z.object({
    notifications: z.array(monitorNotificationSchema),
});

// --- Parameter & Query Validations ---
...
export const createMonitorBodyValidation = z.object({
    ...
    type: z.enum(MonitorTypes, { errorMap: () => ({ message: "Invalid monitor type" }) }),
    ...
    // FIXED: Changed from z.array(z.string()) to use the new object schema
...
export const editMonitorBodyValidation = createMonitorBodyValidation.partial().extend({
    // Explicitly keeping these defaults/constraints if partial() isn't enough
    statusWindowSize: z.number().min(1).max(20).default(5),
    statusWindowThreshold: z.number().min(1).max(100).default(60),
...
const importedMonitorSchema = z.object({
    ...
    // FIXED: Updated for import logic compatibility
    notifications: z.array(monitorNotificationSchema).default([]),
	escalationMinutes: z.number().optional(),
	escalationNotifications: z.array(z.string()).optional(),
    secret: z.string().optional(),
	...
	lastEscalationSent: z.string().optional(),
