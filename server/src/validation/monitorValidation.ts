...
export const createMonitorBodyValidation = z.object({
	...
	// New escalation notification menu START
	escalationAfterMinutes: z.coerce.number().int().min(0).max(43200).optional().default(0),
...
export const editMonitorBodyValidation = z.object({
	...
	interval: z.number().optional(),
	notifications: z.array(z.string()).optional(),
	escalationAfterMinutes: z.coerce.number().int().min(0).max(43200).optional(),
...
const importedMonitorSchema = z.object({
	...
	escalationAfterMinutes: z.number().int().min(0).max(43200).default(0),
