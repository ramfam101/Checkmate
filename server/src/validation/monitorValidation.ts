...
export const escalationRuleValidation = z.object({
	afterMinutes: z.number().int().min(1, "Time must be at least 1 minute").max(1440, "Time cannot exceed 24 hours"),
	notificationIds: z.array(z.string().min(1)).min(1, "At least one notification is required"),
...
export const createMonitorBodyValidation = z.object({
	...
	escalationRules: z.array(escalationRuleValidation).optional(),
});
...
const importedMonitorSchema = z.object({
	...
	escalationRules: z.array(escalationRuleValidation).default([]),
