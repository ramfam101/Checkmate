...
export class EmailService implements IEmailService {
	...
	sendEmail = async (to: string, subject: string, html: string, transportConfig?: EmailTransportConfig) => {
		...
		// Log the configuration being used (excluding password)
		this.logger.info({
			message: "Email configuration being used",
			service: SERVICE_NAME,
			method: "sendEmail",
			details: {
		...
		if (!systemEmailHost || !systemEmailPort) {
			this.logger.error({
				message: "Missing required email configuration (host or port)",
				service: SERVICE_NAME,
				method: "sendEmail",
				details: { host: systemEmailHost, port: systemEmailPort },
			});
			return false;
		}

		if (!systemEmailAddress) {
			this.logger.error({
				message: "Missing required email configuration (systemEmailAddress)",
				service: SERVICE_NAME,
				method: "sendEmail",
			});
			return false;
		...
		this.logger.info({
			message: "Creating nodemailer transporter with config",
			service: SERVICE_NAME,
			method: "sendEmail",
			details: {
		...
		try {
			this.logger.info({
				message: "Verifying email transporter connection...",
				service: SERVICE_NAME,
				method: "sendEmail",
			});
